...
# Add the arguments
parser.add_argument("url", type=git_url_type, help="the git url to clone")
...
if clone_path.exists():
    # Check if the directory is empty, since git can clone in that case
    if any(clone_path.iterdir()):
        if not args.quiet:
            print("Error: target directory already exists and is not empty.\n\nAccess it using:", file=sys.stderr)
